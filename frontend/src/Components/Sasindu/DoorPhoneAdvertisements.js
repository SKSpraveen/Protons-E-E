...
function DoorPhoneAdvertisements() {
    const [doorphoneAdvertisements, setDoorPhoneAdvertisements] = useState([]);
        ...
        axios.get("http://localhost:8070/Advertisement")
            .then((response) => {
                if (response.data.success) {
                    const filteredAds = response.data.advertisements.filter(ad => ad.type === "Door Phone");
                    setDoorPhoneAdvertisements(filteredAds);
                } else {
    ...
    return (
        <div>
            ...
            <div style={{backgroundColor:"rgba(16, 16, 16, 0.926)",height:"56px"}}>
                <h1 style={{fontSize:"35px",padding:"4px"}}> Door Phone</h1>
            </div>
            <br />
            <div className="gallery">
                {doorphoneAdvertisements.map((ad) => (
...
export default DoorPhoneAdvertisements;
